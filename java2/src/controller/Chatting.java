...
public class Chatting implements Initializable {
    ...
    @FXML
    private TableView<Room> roomtable; // 제네릭 Room 선언 안하고 ? 집어넣으면 실행 x 
    ...
    public void midshow() { //msg 메소드 : 입력창에서 엔터쳤을떄 / send 메소드 : 전송버튼 눌렀을때
    	//1. 테이블뷰에서 샌택된 방번호의 접속 된 회원명단 가져오기 
    ...
    void connect(ActionEvent event) {
    	if( btnconnect.getText().equals("채팅방 입장") ) {// 만약에 버튼의 텍스트가 "채팅방 입장" 이면 
        	...
        	txtroomname.setDisable(true); //채팅방이름 입력창 사용 
        	btnadd.setDisable(true);
        	roomtable.setDisable(true);
        	...
        	//2. 만약에 방접속 명단이 0명이면 방삭제
        	//2-2 만약에 방이 삭제되면서서버 소켓 종료
